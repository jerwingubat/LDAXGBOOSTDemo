...
def fuzzy_map_columns(df_columns, target_features, threshold=30):
    mapping = {}
    for feature in target_features:
        ...
        if score >= threshold:
            ...
            aliases = {
                "Library_Usage_Hours": ["Library", "Library Hours", "Study Hours"],
                "Year_Level": ["Year", "Level", "Year Level", "Current Year"],
                "School_Environment_Feedback": ["Feedback", "Comments", "Behavioral Data","Environment", "Opinion", "School Feedback", "Behavior", "Behavioral"],
                "Scholarship_Status": ["Scholarship", "Is Scholar", "Scholarship Status","Grantee", "Has Scholarship"],
                "Attendance_Rate": ["Attendance", "Attendance Percent", "Presence Rate"]
            }
            if feature in aliases:
                extended = df_columns + aliases[feature]
                best_alt, alt_score = process.extractOne(feature, extended, scorer=fuzz.token_sort_ratio)
                if best_alt in df_columns and alt_score >= 65:
                    mapping[feature] = best_alt
                else:
                    pass
            else:
                pass
...
def batch_predict():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            missing = [k for k in feature_names if k not in column_mapping]
            if missing:
                flash(f'Missing mapped columns for: {missing}')
                return redirect(request.url)
            ...
            return render_template('batch_predict.html', table=table, columns=original_df.columns)
        else:
